 ...
 * to preserve complexity. If (3) is expected/required to run, make sure to use
 ...
 * This is useful for all those codepaths where we rely on the mark
 * (e.g., MARK_MAGIC_OVERLAY).
 * In XDP we simply skip this part, given the mark is not available.
 */
 ...
 */
	 ...
	 * For IPv6, let's consider just the first header rather than
	 * the whole ipv6_hdrlen() as not needed.
	 */
	if (!proto)
		proto = ctx_get_protocol(ctx);
	switch (proto) {
